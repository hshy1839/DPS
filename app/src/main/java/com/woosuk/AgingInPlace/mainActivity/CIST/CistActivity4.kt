    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nextButton.setOnClickListener {
            ...
            if (emptyFields.isNotEmpty()) {
                // 빈 필드가 있으면 알림 표시
                Toast.makeText(this@CistActivity4, "빈 칸을 채워주세요", Toast.LENGTH_SHORT).show()
                    ...
                    if (questions != null && questions.isNotEmpty()) {
                        ...
                        if (filteredQuestions.isNotEmpty()) {
                            ...
                            for (question in filteredQuestions) {
                                ...
                                val questionImageView = ImageView(this@CistActivity4).apply {
                                    // 이미지 URL로부터 이미지를 로드합니다
                                    val imageUrl = question.image_url // 질문에서 이미지 URL을 가져옵니다
                                    if (imageUrl != null) {
                                        Glide.with(this@CistActivity4) // Glide 라이브러리를 사용하여 이미지 로드
                                            .load(imageUrl)
                                            .into(this)
