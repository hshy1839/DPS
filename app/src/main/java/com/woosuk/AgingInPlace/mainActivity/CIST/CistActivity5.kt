    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nextButton.setOnClickListener {
            ...
            if (emptyFields.isNotEmpty()) {
                // 빈 필드가 있으면 알림 표시
                Toast.makeText(this@CistActivity5, "빈 칸을 채워주세요", Toast.LENGTH_SHORT).show()
                return@setOnClickListener // 함수를 종료하여 다음 단계로 진행하지 않음
            }
            showProgressBar()
            hideProgressBar()
            postCistResponses()
            val intent = Intent(this@CistActivity5, CistActivity6::class.java)
            ...
            override fun onResponse(call: Call<List<CistQuestionResponse>>, response: Response<List<CistQuestionResponse>>) {
                if (response.isSuccessful) {
                    ...
                    if (questions != null && questions.isNotEmpty()) {
                        val filteredQuestions = questions.filter { it.type == "집행 기능" }
                        if (filteredQuestions.isNotEmpty()) {
                            ...
                            for (question in filteredQuestions) {
                                if (!displayedTitles.contains(question.title)) {
                                    ...
                                    val questionTitle = TextView(this@CistActivity5).apply {
                                        text = question.title
                                        textSize = 18f
                                ...
                                val questionText = TextView(this@CistActivity5).apply {
                                ...
                                val questionImageView = ImageView(this@CistActivity5).apply {
                                    // 이미지 URL로부터 이미지를 로드합니다
                                    val imageUrl = question.image_url // 질문에서 이미지 URL을 가져옵니다
                                    if (imageUrl != null) {
                                        Glide.with(this@CistActivity5) // Glide 라이브러리를 사용하여 이미지 로드
                                            .load(imageUrl)
                                            .into(this)
                                ...
                                // EditText 추가
                                val editText = EditText(this@CistActivity5).apply {
                                    hint = "여기에 입력하세요"
                                    setTextColor(android.graphics.Color.BLACK)
